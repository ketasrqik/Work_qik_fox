...
int fun_up_scan(char *argv[]) 
{
    ...
    if (ret = cl_init(CL_INIT_DEFAULT)) {
        printf("Cant initialize libclamav: %s\n", cl_strerror(ret));
        goto done;
    }

    /* Creating a new scan engine */
    if (!(engine = cl_engine_new())) {
        printf("Cant initialize engine\n");
        goto done;
    }

    /* Database loading */
    char *ptr = cl_retdbdir();
    if ((ret = cl_load(ptr, engine, &info.sigs, dboptions))) {
        printf("%s\n", cl_strerror(ret));
        free(ptr);
        goto done;
    }
    ...
    if (out == 0) {
        ...
        if ((ret = cl_scanfile(file, &virname, NULL, engine, &options)) == CL_VIRUS) {
