...
int fun_up_scan(int argc, char *argv[])
{
    ...
    const char up[] = "update"; 

    filename = argv[0];
    
    printf("Entered in programm\n");
    ...
    if (out == 0) {
        printf("Standard database update\n");
        printf("Yes..\n");
        ...
        printf("Completed list of db opt-ins\n");
        ...
        printf("Completed list of db opt-out\n");
        ...
        printf("Completed select database fro official list\n");
        ...
        // Initialzing libclamav
        if ((har = cl_init(CL_INIT_DEFAULT))) {
        ...
        if ((har = cl_engine_compile(engine)) != CL_SUCCESS) {
            printf("Building engine failed: %s\n", cl_strerror(har));
            com = 8;
            goto done;
        }
        // Scanning file
        if ((har= cl_scanfile(filename, &virname, NULL, engine,
                               &options)) == CL_VIRUS) {
            ...
            if (har == CL_CLEAN) {
                printf("No virus detected in file\n");
                com = 3;
                goto done;
            } else {
