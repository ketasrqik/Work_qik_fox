...
VOID __stdcall DisplayUsageA(void); 
...
//
int SvcConfig(const TCHAR *argv[])
{
    
    StringCchCopy(szkCommand, 10, argv[0]);
    StringCchCopy(szkSvcName, 80, argv[1]);

    if (lstrcmpi(szkCommand, TEXT("query")) == 0)
        DoQuerySvc(); 
    else if (lstrcmpi(szkCommand, TEXT("describe")) == 0)
        DoUpdateSvcDesc();
    else if (lstrcmpi(szkCommand, TEXT("disable")) == 0)
        DoDisableSvc(); 
    else if (lstrcmpi(szkCommand, TEXT("enable")) == 0)
        DoEnableSvc();
    else if (lstrcmpi(szkCommand, TEXT("delete")) == 0)
        DoDeleteSvc(); 
    else {
